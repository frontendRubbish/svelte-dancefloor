<script lang="ts">
  ...
  import track01 from './assets/track01.mp3';
  import track02 from './assets/track02.mp3';
  ...
  function startPlay() {
    console.log('startPlay', track01, player);
  ...
  afterUpdate(() => {
    ...
    if( lastDancing !== dancing && !dancing) {
      player.pause();
...
<div class="start {(dancing) ? 'start--hidden' : ''}">
  <audio bind:this={player} loop>
    <source src={trackSource} type="audio/mpeg">
  </audio>
  <button class="start__button" on:click="{startPlay}"><svg
...
<button class="start__button" on:click="{startRiot}"><svg
   xmlns="http://www.w3.org/2000/svg"
   version="1.0"
   width="225.67999"
   height="225.687"
